...
export default function LocationSearchScreen() {
  ...
  const handlePlaceSelection = async (placeId: string) => {
    try {
      ...
      alert('Failed to get details for the selected location. Please try again.');
    }
  };
  ...
  const renderAddressItem = ({ item }: { item: Address }) => {
    const addressIcon = item.type === 'home' ? 'home' : item.type === 'work' ? 'work' : 'location-on';
    ...
    return (
      ...
      >
        <View style={styles.addressIconContainer}>
          <IconSymbol name={addressIcon} size={24} color="#8A3FFC" />
        </View>
  ...
  const renderSearchResultItem = ({ item }: { item: any }) => (
    ...
    >
      ...
      <View style={styles.searchResultTextContainer}>
        ...
        <Text style={styles.searchResultSecondaryText}>{item.secondaryText}</Text>
      </View>
    </TouchableOpacity>
  );
  ...
  return (
    <View style={styles.container}>
      ...
      <TouchableOpacity
        ...
        disabled={isLoading}
      >
        <IconSymbol name="my-location" size={24} color="#8A3FFC" />
        <Text style={styles.currentLocationText}>Use your current location</Text>
        {getCurrentLocationMutation.isPending && (
      ...
      {addresses && addresses.length > 0 && (
        <>
          ...
          {isLoadingAddresses ? (
            <ActivityIndicator size="small" color="#8A3FFC" style={styles.addressesLoader} />
          ) : (
            <FlatList
              data={addresses}
      ...
      {searchQuery.length > 2 && (
        <>
          {searchPlacesMutation.isPending ? (
            <ActivityIndicator size="small" color="#8A3FFC" style={styles.resultsLoader} />
          ) : (
            <FlatList
              data={searchResults}
